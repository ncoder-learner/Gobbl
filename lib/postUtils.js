...
import { notifyFriendPost } from './notifications';
...
export async function notifyFriendsOfPost(mealName) {
  try {
    ...
    const friendIds = (friendRows || []).map(row => row.requester_id === user.id ? row.addressee_id : row.requester_id).filter(Boolean);
    await Promise.all(friendIds.map(friendId => notifyFriendPost(friendId, actorUsername, mealName)));
  } catch (error) {
    console.warn('[postUtils] friend post notification failed:', error?.message ?? error);
  }
}
