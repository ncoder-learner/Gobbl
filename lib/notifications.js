...
const ID_LUNCH = 'fw-lunch-reminder';
const ID_DINNER = 'fw-dinner-reminder';
const CHANNEL_DEFAULT = 'default';
const CHANNEL_SOCIAL = 'social';
const CHANNEL_REMINDERS = 'reminders';
const CHANNEL_STREAK = 'streak';
const tokenRegistrationByUser = new Map();
...
export async function registerPushTokenForUser(userId) {
  ...
  if (tokenRegistrationByUser.has(userId)) return tokenRegistrationByUser.get(userId);

  const registration = registerPushTokenForUserInternal(userId);
  tokenRegistrationByUser.set(userId, registration);
  try {
    return await registration;
  } finally {
    tokenRegistrationByUser.delete(userId);
  }
}

async function registerPushTokenForUserInternal(userId) {
  await ensureAndroidChannels();
...
export async function syncMealWindowNotifications() {
  try {
    ...
    await Promise.all([
      Notifications.cancelScheduledNotificationAsync(ID_LUNCH).catch(() => {}),
      Notifications.cancelScheduledNotificationAsync(ID_DINNER).catch(() => {}),
    ]);

    const lunchHour = 12;
    const lunchMinute = 30;
    const dinnerHour = 18;
    const dinnerMinute = 45;
    ...
    await Promise.all([
      Notifications.scheduleNotificationAsync({
        ...
        content: {
          ...
          body: 'Log lunch before the window closes.',
          sound: true,
          data: { type: 'meal_time_reminder', slotLabel: 'lunch' },
        },
        trigger: {
          type: Notifications.SchedulableTriggerInputTypes.DAILY,
          hour: lunchHour,
      ...
      Notifications.scheduleNotificationAsync({
        identifier: ID_DINNER,
        content: {
          title: '🍽️ Dinner check-in',
          body: 'Log dinner before the window closes.',
          sound: true,
          data: { type: 'meal_time_reminder', slotLabel: 'dinner' },
        },
        trigger: {
          type: Notifications.SchedulableTriggerInputTypes.DAILY,
          hour: dinnerHour,
    ...
    // Notification scheduling is best-effort and must not break the app.
  }
}
